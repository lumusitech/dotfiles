...
Item {
  ...
  function invokePopupDefault(index) {
    ...
    openOrFocusApp(entry)
    dismissPopup(index)
  }

  // Open or focus the application corresponding to the notification sender
  function openOrFocusApp(entry) {
    if (!entry) return
    var appName = entry.app ? String(entry.app) : ""
    var desktopEntry = entry.desktopEntry ? String(entry.desktopEntry) : ""
    var appIcon = entry.appIcon ? String(entry.appIcon) : ""
    var summary = entry.summary ? String(entry.summary) : ""
    var body = entry.body ? String(entry.body) : ""

    if (!appName && !desktopEntry && !appIcon) return

    var helper = Quickshell.env("HOME") + "/.local/bin/omarchy-notification-open-app"
